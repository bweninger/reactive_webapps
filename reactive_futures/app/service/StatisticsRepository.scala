...
class MongoStatisticsRepository @Inject()(reactiveMongoApi: ReactiveMongoApi) extends StatisticsRepository {

  private val StatisticsCollection = "UserStatistics"

  private lazy val collection = reactiveMongoApi.db.collection[BSONCollection](StatisticsCollection)

  override def storeCounts(counts: StoredCounts)(implicit ec: ExecutionContext): Future[Unit] = {
    collection.insert(counts).map {
      lastError =>
  ...
  override def retrieveLatestCounts(userName: String)(implicit ec: ExecutionContext): Future[StoredCounts] = {
    ...
    collection.find(query).sort(order).one[StoredCounts].map {
      _.getOrElse(StoredCounts(DateTime.now, userName, 0, 0))
    }
  } recover {
    case NonFatal(t) => throw CountRetrievalException(userName, t)
  }
}
